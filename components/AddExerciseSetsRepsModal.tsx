...
interface AddExerciseSetsRepsModalProps {
  isVisible: boolean;
  onClose: () => void;
  exercise: {
    id: string;
...
export function AddExerciseSetsRepsModal({
  isVisible,
  onClose,
  exercise,
  onConfirm,
}: AddExerciseSetsRepsModalProps) {
  const { t, i18n } = useTranslation();
  const isRTL = useRTL();

  const [sets, setSets] = useState(3);
  const [repsPerSet, setRepsPerSet] = useState<Array<{ reps: number; weight?: number }>>(
    Array(3).fill(null).map(() => ({ reps: 10, weight: undefined }))
  );
  const [notes, setNotes] = useState('');

  useEffect(() => {
    // Reset form when modal opens for a new exercise
    if (isVisible) {
      setSets(3);
      setRepsPerSet(Array(3).fill(null).map(() => ({ reps: 10, weight: undefined })));
      setNotes('');
    }
  }, [isVisible, exercise.id]);
  ...
  const updateSetsCount = (newSets: number) => {
    ...
    if (newSets > currentSetsData.length) {
      for (let i = currentSetsData.length; i < newSets; i++) {
        currentSetsData.push({ reps: 10, weight: undefined }); // Default values for new sets
      }
  ...
  const handleConfirm = () => {
    ...
    const processedRepsPerSet = repsPerSet.map(setData => ({
      reps: setData.reps,
      weight: setData.weight === undefined ? null : setData.weight // Explicitly convert undefined to null
    }));
    ...
    onConfirm({
      ...
      sets,
      reps: processedRepsPerSet, // Use the processed array
  ...
  const styles = StyleSheet.create({
    ...
    weightInput: {
      ...
      borderWidth: 1,
      borderColor: '#E5E7EB',
      paddingHorizontal: 12, // Changed from 8 to 12 to match repsInput
      fontSize: 16,
      color: '#111827',
      textAlign: 'center',
    },
        ...
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>
              {t('exercisesListScreen:addToWorkoutPlan')}
            </Text>
