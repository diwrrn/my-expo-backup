...
export default function PlanDetailsScreen() {
  ...
  const handleDeleteExercise = async () => {
    if (!exerciseToDelete || !plan || !user) return;

    try {
      // Optimistic UI update: Immediately remove the exercise from local state
      const originalExercises = plan.exercises; // Save current state for potential rollback
      const updatedExercises = plan.exercises.filter((_, index) => index !== exerciseToDelete.index);
      const updatedPlan = {
        ...plan,
      ...
      await FirebaseService.removeExerciseFromWorkoutPlan(user.id, plan.id, exerciseToDelete.exerciseId);
      
    } catch (error) {
      // Rollback UI if Firebase operation fails
      updatePlan({ ...plan, exercises: originalExercises });
  ...
  const handleUpdateExercise = async (updatedData: {
    ...
    try {
      ...
      const updatedExercises = plan.exercises.map((ex, idx) => {
